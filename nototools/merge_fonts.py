...
Sample Usage:
    $ python3 dir_to_merge_fonts_script -d dir_to_source_fonts
"""

"""
filesDic .ttf files will be merged 
with baseMergeMap's NotoSans-#{fontWeight}.ttf. Modify 
as needed. The file output is the current shell's directory
...
"""
# do not provide for chinese, korean, japanese (otf issue)
# spanish, vietnamese, russian, tagalog (included in noto)
alphabets = [
    'Devanagari',
    'Khmer',
    'Armenian',
...
# dictionary for files to merge with base files
filesDic = {}
# modify for additional weights provided they exist in Noto Sans
baseMergeMap = {
...
for name in alphabets:
    filesDic[name] = {}
    for weight in baseMergeMap.keys():
        filesDic[name][weight] = f'NotoSans{name}-{weight}.ttf'
...
def build_valid_filenames(files, directory=directory):
    files = list(files)
...
def main(newName, files):
    ...
    print('Please check the result at %s.' % os.path.abspath(
        os.path.realpath(newName)))

# generates font files from current terminal directory 
if __name__ == '__main__':
    # main()
   for key in filesDic.keys():
    for weight in baseMergeMap.keys():
        files = [baseMergeMap[weight], filesDic[key][weight]]
        newName = f'NotoSans{key}-{weight}.ttf'
        main(newName, files)
